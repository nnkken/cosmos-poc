...
import (
	...
	app "github.com/nnkken/cosmos-poc"
)

var DefaultNodeHome = os.ExpandEnv("$HOME/.pocd")
...
func main() {
	...
	executor := cli.PrepareBaseCmd(rootCmd, "POC", DefaultNodeHome)
...
func newApp(logger log.Logger, db dbm.DB, traceStore io.Writer) abci.Application {
	return app.NewPocApp(logger, db)
}
...
func InitCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command {
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			err = gaiaInit.ExportGenesisFile(genFile, chainID, []tmtypes.GenesisValidator{validator}, appState)
			if err != nil {
				return err
			}

			cfg.WriteConfigFile(filepath.Join(config.RootDir, "config", "config.toml"), config)
			fmt.Printf("Initialized pocd configuration and bootstrapping files in %s\n", root)
			return nil
		},
	}
...
$ pocd add-genesis-account cosmos1tse7r2fadvlrrgau3pa0ss7cqh55wrv6y9alwh 1000STAKE,1000nametoken
`),
...
func SimpleAppGenTx(cdc *codec.Codec, pk crypto.PubKey) (appGenTx, cliPrint json.RawMessage, validator tmtypes.GenesisValidator, err error) {
	...
	bz, err = cdc.MarshalJSON(map[string]string{
		"secret": secret,
	})
	if err != nil {
		return
	...
	return appGenTx, cliPrint, validator, nil
}